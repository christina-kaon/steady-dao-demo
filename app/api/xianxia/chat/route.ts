...
function buildPerceptionPacket(input: string, inputKind: string, audienceIds: string[]): PerceptionPacket {
  ...
  const visibleAction = /(?:到达|抵达|来到|赶到|回到|返回|前往|赶往|出发|动身|下山|上山|进城|出城|登岸|穿过|翻越|登上|跳下|落地|传送|挪移|御剑|腾云|走|跑|飞|拿|取|拔|看|查|翻|抱|牵|吻|推|拉|布阵|放出|跟上|起身|坐下|站起|躺下|跪下|蹲下|递|交|扔|摸|杀|斩|砍|刺|击|打|制住|捆|封|救|治疗|追|躲|靠近|离开|进入|打开|关上|放下|收起|藏起|毁掉|烧掉|转身|点头|摇头|等到|过了一夜|次日|翌日|天亮|入夜|三日后|几天后)/;
...
function inferAssertedLocation(input: string, inputKind: string) {
  ...
  const actionTravel = inputKind === "action"
    ...
    : null;
  const candidate = (completed ?? actionTravel)?.replace(/(?:这里|此地|了)$/u, "").trim();
...
5. 严守现场空间连续性：scene.location与scene_memory是已经发生的当前状态，优先于compiled_location_fallback。玩家在player_perception.seen中明确完成“到达、抵达、进入、离开、返回、时间过去”等动作时，只要没有既有物理规则具体阻止，就必须承认并从新地点或新时间继续；不得让Prompt 2的静态地点把人物传回旧场景。没有明确移动时，院落不能突然写成屋内，室外人物不能无过渡出现在窗边或室内炉火旁。
