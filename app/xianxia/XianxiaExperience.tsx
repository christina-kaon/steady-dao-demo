      ...
      <section className="xx-transcript" ref={transcriptStartRef}>
        {displayedMessages.map((item) => {
          ...
          if (item.type !== "dialogue" || !item.person) {
            return <p className="xx-narration" key={item.id}>{renderRichText(item.text ?? "")}</p>;
          }
          const character = characterById.get(item.person);
          return (
            <article className={`xx-npc-event xx-${item.type}`} key={item.id}>
              <button onClick={() => character && setActiveCharacter((current) => current?.id === character.id ? null : character)}>{character ? <CharacterPortrait character={character} /> : <i>{item.person.slice(0, 1)}</i>}</button>
              <div>
...
function MediaCueCard({ cue }: { cue: XianxiaMediaCue }) {
  if (cue.kind !== "hud" && cue.kind !== "image") return null;
  if (cue.kind === "image") {
    return (
  ...
  return (
    <aside className={`xx-hud-card${"compact" in cue && cue.compact ? " xx-turn-hud" : ""}`} aria-label={cue.title}>
